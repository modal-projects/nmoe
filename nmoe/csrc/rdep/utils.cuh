...
__device__ __forceinline__ dtype_b_t pack2(const dtype_a_t& x, const dtype_a_t& y) {
    RDEP_STATIC_ASSERT(sizeof(dtype_a_t) * 2 == sizeof(dtype_b_t), "Invalid dtypes");
    dtype_b_t packed;
...
__device__ __forceinline__ dtype_t broadcast(dtype_t& ptr, int src_lane_idx) {
    RDEP_STATIC_ASSERT(sizeof(dtype_t) % sizeof(int) == 0, "");
...
barrier_block(int** barrier_signal_ptrs, int rank) {
    ...
    RDEP_DEVICE_ASSERT(kNumRanks <= blockDim.x);
    ...
    while (true) {
        ...
        if (clock64() - start_time > NUM_TIMEOUT_CYCLES and thread_id < kNumRanks) {
            printf("RDEP timeout check failed: rank = %d, thread = %d, value = %d)\n", rank, thread_id, value);
            trap();
        }
    }
    __syncthreads();
...
__forceinline__ __device__ T warp_reduce(T value, Op op) {
    RDEP_STATIC_ASSERT(kNumLanesPerGroup == 32 or kNumLanesPerGroup == 16 or kNumLanesPerGroup == 8 or
